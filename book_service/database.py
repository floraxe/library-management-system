"""Book database helpers, SQLite for tests / MySQL for deployment.

图书数据库辅助函数：单元测试继续使用 SQLite（不改变原有 pytest 行为），
...
def get_mysql_connection(database_url: str) -> _MySQLConnectionProxy:
    ...
    if pymysql is None:
        raise RuntimeError(
            ...
            "add pymysql to book_service/requirements.txt"
        )
    parsed = urlsplit(database_url)
    database_name = parsed.path.lstrip("/")
    if not database_name:
        raise RuntimeError("MySQL DATABASE_URL must include a database name, e.g. mysql://root:root@mysql:3306/library_books")
...
def init_db() -> None:
    """Create the book schema / 创建图书数据结构。"""
    database_url = get_database_url()
    scheme = urlsplit(database_url).scheme
    ...
    with get_connection() as connection:
        if scheme == "mysql":
            ...
            _create_index_if_absent(connection, "CREATE INDEX idx_books_title ON books(title)")
            _create_index_if_absent(connection, "CREATE INDEX idx_books_author ON books(author)")
        else:
            connection.execute(
                """
                CREATE TABLE IF NOT EXISTS books (
            ...
            connection.execute("CREATE INDEX IF NOT EXISTS idx_books_author ON books(author)")
